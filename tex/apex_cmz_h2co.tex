...
under extreme physical conditions.  It has an apparent star formation rate per
unit `dense'
gas below that of normal molecular clouds and disk galaxies.}
{To measure the temperature of gas in the Central Molecular Zone (CMZ)and determine what drives it.}
{We have mapped the inner 300 pc of the CMZ  in para-\formaldehyde
...
the  `dust ridge' \citep{Lis1991a,Sofue1995a,Molinari2011a} within our Galaxy's
CMZ has attracted
...
used as a proxy for the dense gas temperature.  While the discrepancy is not
...
APEX band.  All detected lines are identified.  The grey background indicates
...
included CO, O, C+, and C as line coolants\footnote{We assume a very low
abundance for C+, $X_{C+}=5\ee{-10}$, assuming that cloud shielding will
prevent photoionization and collisions with \hh will destroy CR-generated C+
rapidly, which is confirmed by the \citet{Nelson1999a} chemical model.
...
dendrogram-extracted clumps within 35 \kms and 12 pc of the orbital path.  The
darker symbols represent `leaves'
...
ratios toward The Brick, but our observations are on larger scales.  The
...
area (it must have a small filling factor).  A small filling factor is
inconsistent with the observed large extent of the emission
unless the emission is also extremely patchy on small scales.
Patchy, low-filling-factor, high-column gas may exist, but it is unlikely because 
the high observed turbulence should prevent the gas from becoming optically
thick if the filling factor is low.  We discuss this topic further in Section
...
lower limit closer to $T\gtrsim120$ K.
...
in the scale of this uniformity and in the association of these temperatures
with particular physical mechanisms, but overall we confirm those early results.
There is warm ($T\gtrsim60$ K) dense gas pervading the CMZ out to a radius
$\sim200$ pc.
...
lettered clouds on the path to Sgr B2) have consistent thermal properties and
fall along a stream of progressively increasing temperature as a function of
age along the predicted orbits.  

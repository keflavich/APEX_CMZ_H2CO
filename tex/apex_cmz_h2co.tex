...
The central $\sim100$ pc `ring' or `dust ridge' \citep{Lis1994a,Sofue1995a,Molinari2011a}
within our Galaxy's central molecular zone (CMZ) has attracted a great deal of
attention recently,
...
linewidths $FWHM\gtrsim5$ \kms and local clouds $FWHM \lesssim 2$ \kms, but this only provides a qualitative distance, distinguishing
nearby ($d\sim1-5$ kpc) clouds from clouds within the central kiloparsec.
...
positive latitudes is known to be in the foreground of the CMZ both because it
is seen as an infrared-dark absorption feature \citep{Yusef-Zadeh2009a} and
because Sgr B2 has a closer parallax distance than Sgr A* \citep{Reid2009a}.
...
using the method described in Section \ref{sec:signal}.  Selected region names
are labeled in (b).}
...
detected while the \threeohthree line is.  If the high column density gas in,
e.g., G0.253+0.016 had a lower
...
temperatures, as can be seen in Figure \ref{fig:parsonbrightness} (rightmost
column), in which the modeled line brightnesses are both nearly constant above
$T\gtrsim150$ K.
...
{(a) A Galactic longitude-velocity diagram made by averaging the smooth ratio
cube along Galactic latitude and using the $n=10^4$ \percc curve from Figure
\ref{fig:ratiovstem}.  (b) A Galactic longitude-velocity map of peak

...
form\footnote{\texttt{doi:10.7910/DVN/27601}, also available from CDS via
anonymous ftp to cdsarc.u-strasbg.fr (130.79.128.5) or via
\url{http://cdsweb.u-strasbg.fr/cgi-bin/qcat?J/A+A/}}.}
...
\methylcyanide 12-11, that are in the observed band but were detected only in
...
identified.  The feature overlapping the \ceighteeno
2-1
...
\url{www.apex-telescope.org/documents/public/APEX-MPI-MAN-0012.png}
\url{www.apex-telescope.org/documents/public/APEX-MPI-MAN-0013.png} }.
...
% figure_ratio_maps
\RotFigureTwoAA
%{figures/big_H2CO_322221_to_303202_smooth_bl_integ.png}
...
%{figures/big_maps/big_lores_tmap_withcontours.png} % tmap_figure
...
% individual_spectra
\Figure{figures/simple/c:G0.38+0.04_fit_4_lines_simple.png}
...
% dendrotem_plots
\Figure{figures/dendrotem/ratio_vs_temperature_modeloverlay.png}
...
%\RotFigureTwoAA
\RotFigureThreeAA
%{figures/big_maps/big_lores_smooth_tmap_withcontours.png}
...
%\RotFigureTwoAA
%{figures/big_maps/pv_tmap_smooth_weight_masked.png}
%{figures/big_maps/pv_peaksn_smooth.png}
...
% This is no longer a small fraction; it's now more like 1/3
% MAJOR TODO: re-run despotic_heating now that sources have been reextracted
% The small fraction of sources with measured
...
%
% There are some dendrogram sources with measured temperatures higher than that
% from the turbulence-only prediction.  For these clumps, either the turbulent
% heating rate has been underestimated because the velocity dispersion has been
% underestimated or the clump size overestimated, or another heating mechanism
% is significant.
...
\FigureTwo
{figures/chi2_temperature_vs_turbulenttemperature_fieldsandsources.png} %parameter_comparisons
%{figures/dendrotem/temperature_vs_turbtemperature.png} # dendrotem_plots
{figures/dendrotem/temperature_vs_Despoticturbtemperature.png} % dendrotem_plots
...
To explore the possibility that our cooling rates are underestimated, we
...
\FigureTwo
{figures/chi2_temperature_vs_higaltemperature_fieldsandsources.png} % parameter_comparisons
{figures/dendrotem/temperature_vs_dusttem.png} % dendrotem_plots
...
%Nonetheless, if the trend is real, it may indicate that additional turbulence
%is driven on closest passage, decaying over a timescale of $\sim1$ Myr to heat
%the gas.  The cooler gas on the far side does not obviously fit into this
%picture, and we suggest in Section \ref{sec:coolstream} that it is not
%affiliated with the orbit (but the existence of such a cool stream does
%\emph{not} challenge the model directly).
...
\citet{Ott2014a} measured the \ammonia (1,1) and (2,2) lines over a subset of
...
have lower critical densities than the \formaldehyde 3-2 transitions by a few
...
%{figures/big_maps/lores_smoothdens1e4_weighted_masked_tmap_withcontours.png}
...
% % backstream_pv
% \RotFigureTwoAA
% {figures/orbits/mydrawnpathpv_on_H2CO_TemperatureFromRatio_weighted.png}
% {figures/orbits/mydrawnpath_on_H2CO_TemperatureFromRatio_weighted_masked.png}
...
%this is not correct!
% In the context of the \citet{Kruijssen2014c} and \citet{Krumholz2015b}
% episodic star formation model for the CMZ, the global gas-dust temperature
% difference may indicate that the CMZ is currently near a star formation
% minimum, whereas the other galaxies have high star formation rates and hence
% CRIRs.
...
\citep{Offner2013b,Hopkins2013a}, it should be possible to find sub-Jeans-mass
...
In the hand-extracted source table, three types of region are included: 
\begin{enumerate}
...
% param_plots
\Figure{figures/param_fits/c:G0.38+0.04_col_tem_0_parameter_constraints.png}
...
\Figure{figures/param_fits/c:G0.38+0.04_dens_col_0_parameter_constraints.png}
...
\Figure{figures/param_fits/c:G0.38+0.04_dens_tem_0_parameter_constraints.png}
{The constraints in density-temperature parameter space.
See Figure \ref{fig:coltemconstraints} for details.}
{fig:denstemconstraints}{0.5}{7in}

\Figure{figures/param_fits/c:G0.38+0.04_0_h2coratio.png}
...
\Figure{figures/param_fits/c:G0.38+0.04_oneD_0_parameter_constraints.png}
...
%{figures/big_maps/big_lores_smooth_tmap_withcontours.png} % tmap_figure
...
\input{tables/line_props_excerpt.tex}
\input{tables/dendro_props_excerpt.tex}

...
a great deal of attention recently, as it contains most of the gas in the CMZ
and has a large, but unrealized, star-forming potential
...
the star formation rate is lower
\citep{Longmore2013b,Immer2012a,Yusef-Zadeh2009a,Murray2010a}.

Hypotheses for the reduced CMZ star formation rate include a raised star
formation threshold from enhanced turbulence and stochasticity resulting from a
modified threshold.  \citet{Kruijssen2014c} concluded that the
...
linewidths $FWHM\gtrsim5$ \kms and local clouds $FWHM \lesssim 2$ \kms, but
this only provides a qualitative distance, distinguishing nearby ($d\sim1-5$
kpc) clouds from clouds within the central kiloparsec.
Additionally, there is significant velocity confusion within the Galactic
center: many clouds are seen to overlap in velocity while they are not
necessarily at the same distance
\citep{Jones2012a,Oka2012a}.  Within the molecular ring, the `dust ridge' at
positive latitudes is known to be in the foreground of the CMZ because it
is seen as an infrared-dark absorption feature \citep{Yusef-Zadeh2009a}.

Prior gas temperature measurements primarily used the popular ammonia
(\ammonia) metastable inversion transition thermometer
\citep{Guesten1981a,Ott2014a}, which is sensitive to moderate density gas
\citep[$n(\hh) \sim 10^3-10^4$ \percc;][]{Shirley2015a}.  While this
thermometer is generally reliable in cold regions, the population of the higher
energy states - those sensitive to temperatures $T\gtrsim60$ K - may be
affected by `formation heating', a mechanism recently discovered to affect much
of the CMZ gas in which the excess energy from molecule formation leaves a
significant fraction of the molecules in a rotationally excited state
\citep[][]{Mills2013a,Lis2014a}.  
...
temperature is readily determined via continuum mapping and therefore is often
used as a proxy for the gas temperature, but the dust temperature itself is
hydrodynamically unimportant.  \citet{Ao2013a} used the para-formaldehyde (\para) 220 GHz
...
gas and dust temperature, but they only covered the inner 75 pc.
...
windows.  The detected lines were \para \threeohthree 218.22219 GHz, \para
\threetwotwo 218.47563 GHz, \para \threetwoone 218.76007 GHz, SiO 5-4 217.10498
GHz, \methanol \fourtwotwo 218.44005 GHz OCS 18-17 218.90336 GHz, HC$_3$N 24-23
...
\ref{fig:integ303321}a for the \threeohthree map masked with the above method
in and Figure \ref{fig:integ303321}b for the \threetwoone map
...
10^{-9.08\pm1}$, allowing for dramatic uncertainty in
...
to both panels of Figure \ref{fig:temmapnosmooth} to make the low-reliability regions
...
regions, we have measured high temperatures ($T>40$ K).  If the high column
density gas in,
e.g., G0.253+0.016 had a lower gas temperature, our data would be sensitive
enough to measure temperature down to $T\sim20$ K.  The higher temperatures
...
\label{sec:heating}
Supersonic turbulence decays, resulting in direct gas heating
\citep{Falgarone1995a, Pan2009a}.  The energy dissipation rate is proportional
to the gas velocity dispersion cubed, which implies that the temperature
of the gas should be strongly correlated with the observed line width.
However, Figure \ref{fig:temvsfwhm_regions} shows no such correlation.  

Following \citet{Ao2013a}, we determine the expected gas kinetic temperature
by assuming that the heating and cooling processes are in equilibrium.
We expand upon their analysis by solving for the temperature assuming that
both cosmic ray ionization and turbulent heating are present and that the dust
temperature is nonzero ($T_D=25$ K).  We have used the same cosmic ray heating rate
...
kinetic temperature; the associated can be found in the file \texttt{heating.py}
...
\ref{fig:temvsfwhm_regions}.  In the plotted curves, we have assumed a cloud
scale $L=5$ pc, a gradient $dv/dr = 5$ \kms \perpc (the gradient and length
scale are entirely degenerate), and a cosmic ray ionization rate (CRIR)
$\zeta_{CR}=10^{-17}$ \pers, with exceptions noted in the legend.
...
to be genuinely cool and broad.  Perhaps the likeliest explanation for these
...
(a) The hand-selected regions as described in Figure \ref{fig:}.  Triangles
...
{\todo{Add Mach number labels.} (a) Radial plots centered on the southwest portion of `The Brick'.
...
highest temperatures outside of Sgr B2, with $T>100$ K.
...
\citet{Kruijssen2015a} orbit.  The possible linear increase in temperature over the
first $\sim0.5$ Myr seems to support the \citet{Kruijssen2015a} hypothesis that
...
trend.  The symbol sizes are proportional to the \para \threeohthree brightness
...
cloud.  In this region, the \ammonia temperature ranges from the high
...
sufficient to explain the observed gas temperatures. 
...
structure observed in the CMZ molecular clouds (see Figure
\ref{fig:brickradial}).  If supersonic shocks within
...
Brick has very little star formation at present \citep{Rathborne2014b}, so the
temperature cannot be readily explained by star formation, but other clouds,
such as cloud
...
explain the temperature profile.  \todo{Can we use this model to infer the
radiation field?  Does it pass sanity checks?}
% In G1.23-0.08 example, there are no hints of
% active star formation: there are no 24 \um or 70 \um sources and no compact
...
additional smoothed cubes, smoothing in the spatial dimensions with a
...
downsampling are independent).  We then performed

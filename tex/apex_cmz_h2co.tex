...
{To measure the temperature of gas in the CMZ and determine what drives it.}
...
{We produce temperature maps and cubes and publish them in FITS form with
...
heating.  We measure an upper limit on the cosmic ray ionization ray
$\zeta_{CR}<10^{-14}$ \pers. }
...
from local molecular clouds in the past decade, there remain many unanswered
...
\citep[clouds b,c,d,e,f][]{Lis1999a}, ending at Sgr B2, in principle allowing
the study of the time evolution of protocluster clumps.
...
must be higher than we report.} line gradient of 5 \perkmspc, 30 \formaldehyde
...
\ref{fig:temmapnosmooth}a.  Because this map includes the full line-of-sight integrated
...
substantially reduced.  We have also applied a mask to both panels of
...
remnants have lifetimes that are not much longer than a kyr, so the supernovae are
...
If interstellar UV or X-ray heating are relevant for the gas thermal balance,
we would expect to find warmer cloud edges and cooler cloud interiors.  The
same is true if the clouds are centrally condensed, since more efficient dust
cooling can dominate heating processes at very high densities.  If clouds are
internally heated by gravitational collapse and star formation, the cloud
interior could be warmer than its exterior.  
...
to draw firm conclusions about photon-driven heating processes and thermal
balance on different size scales.  
...
by Sgr A*.  The `trend' is also reasonably consistent with a constant temperature over the
...
that the gas be at very high densities.   Given the low brightness temperatures
observed, if
...
which in turn is smaller at higher Mach numbers.  If turbulence is responsible
for setting the core fragmentation scale \citep{Offner2013b,Hopkins2013a}, it
...
code at \url{https://github.com/adamginsburg/APEX_CMZ_H2CO}.  

There is warm ($T\gtrsim60$ K) gas pervading the CMZ out to a radius $\sim200$
pc.  However, there is also significant contrast in gas temperature, with
massive clouds such as The Brick, Sgr B2, and the Sgr A cloud complex
exhibiting temperatures in excess of $T>100$ K.

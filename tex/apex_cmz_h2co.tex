...
physical conditions.  It has an apparent star formation rate per unit `dense'
gas below that of normal molecular clouds and disk galaxies.}
...
temperature ($T\sim60$ K).  Turbulent heating can readily explain the observed
temperatures given the observed line widths.  Cosmic rays cannot explain the
observed variation in gas temperatures, so CMZ cloud temperatures are not
dominated by cosmic ray heating.  The predicted temperature assuming standard
cold cloud chemistry is often higher than the observed temperature, so the
hotter CMZ clouds are able to cool via line emission more efficiently than
local clouds.}
...
\citep{Lis1991a,Sofue1995a,Molinari2011a} within our Galaxy's CMZ has attracted
...
corresponding to $T\lesssim40$ K.  This
...
(n/10^5 \percc) \mathrm{K~kyr}^{-1}$ \citep[][and see below]{Goldsmith2001a}.
...
with previous measurements of the CRIR \citep{Yusef-Zadeh2013b,Goto2013a}.
...
upper limit could be violated if the gas is about an order of magnitude denser
than the dust-derived average density, which is not likely.  This limit leads
to the conclusion that cosmic ray heating is either not dominant in the CMZ or
not uniform, as uniform cosmic ray heating cannot satisfy the requirement for
both high ($>100$ K) and low ($\lesssim40$ K) temperatures to be present in the
CMZ.
...
which is the average between the moment-derived major and minor
...
gaussian following \citet{Rosolowsky2005b} Appendix B.}
$\sigma_v$, the
...
curves given different assumptions as computed by \texttt{DESPOTIC}.  The legend
...
line-of-sight features, supporting the idea that the hand-selected features are
widened by non-turbulent motions.
...
This is entirely plausible, as we have assumed local chemistry (specifically, a
CO abundance $X_{CO}=10^{-4}$ \hh, and negligible abundances for other
coolants), which does not obviously hold under CMZ conditions.  The other
...
\ref{fig:temvstem_regions}.  Triangles indicate regions with lower limits on
...
To examine the possibility that our cooling rates are underestimated, we examine
a particular clump from the catalog, with $n=10^{4.2}$ \percc, $\sigma=8.6
f_g = 14.3$\kms, $L = 2r = 2(0.588)f_g = 1.65$ pc (deconvolved), $T_D=21$ K,
and $T_{D,rad} = 1.2$ K.  This blob has a predicted temperature $T=156$ K,
...
$\Lambda_{O} \approx 0.5 \Lambda_{CO}$.   The predicted equilibrium temperature
...
accounted for when predicting gas temperature.
...
systematic blob extraction of the dendrogram extraction in order to avoid using
the extrapolated correction factors.  This analysis is left for a future work.
...
through the fitted temperature cube following that orbit.  The fit is
imperfect, especially along the green segment, but still very useful for
...
understand the geometry of the CMZ.  Intriguingly, it appears that the
`far-side' clouds (green segment, 1 Myr < $t$ < 2 Myr), are cooler than many of
those on the near side, such as The Brick (red segment).  
...
with time at the first crossing, which is most directly explained by the
\citet{Kruijssen2015a} model.  The possible linear increase in temperature
...
by Sgr A*.  The `trend' is also consistent with a constant temperature over the
same range, so we caution against overinterpretation. Nonetheless, if the trend
is real, it may indicate that additional turbulence is driven on closest
passage, decaying over a timescale of $\sim1$ Myr to heat the gas.
...
\threeohthree)\sim10^4$ \percc \citep[][]{Shirley2015a}.  The temperatures derived from the
...
is an important ingredient in turbulence calculations.  The high temperature
also means that the thermal Jeans mass is about a factor of 5 higher than in
local clouds (assuming $T_{CMZ}=60$ K, $T_{local}=20$ K, $M_{J,CMZ}(n=10^5)
\approx 9$ \msun, $M_{J,local}(n=10^5) \approx 2$ \msun); naively this implies
a larger typical fragmentation scale and perhaps a preference toward forming
high mass stars, though the connection between the Jeans scale and stellar mass
is not at all clear.   However, the high temperatures and high Jeans mass
provide an interesting opportunity to search for other fragmentation
mechanisms.  If turbulence is responsible for setting the core fragmentation
scale \citep{Offner2013b,Hopkins2013a}, it should be possible to find
sub-Jeans-mass fragments, and in the warm CMZ it should be easier than in cold,
less turbulent local clouds.
...
CMZ, but we are able to place an upper limit $\zeta_{CR} < 10^{-14}$ \pers
because a higher CRIR would result in a higher floor temperature than we
observe.  

There are many regions with predicted temperatures substantially higher than
observed.  These regions may have chemistry significantly different from that

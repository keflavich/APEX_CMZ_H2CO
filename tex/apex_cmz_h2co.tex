...
\label{sec:heating}
Following \citet{Ao2013a} and \citet{Papadopolous2010a}, we examine the energy
...
interested in gas at high extinction ($A_V > 10$).  Other processes, e.g.
direct (proto)stellar heating or supernova bubble interaction may be relevant
locally, but cannot heat the gas globally.  \citet{Ao2013a} ruled out diffuse
X-rays as a significant heating source, with an energy density $\sim3$ orders
of magnitude too low to explain observed temperatures.
...
% In the script file example_parplot_constraints.py in the associated
% repository, we examine the systematic uncertainties in the fitting process.
% Using a known density, temperature, and column, we then use the fitting
% process applied to the dendrogram objects and the hand-extracted regions
% to derive our measured temperatures.  

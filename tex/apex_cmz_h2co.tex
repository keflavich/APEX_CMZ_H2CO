...
($r_{eff} = (A/\pi)^{1/2}$, where $A$ is the source area) of the
selected sources as the length scale, the measured linewidth\footnote{We correct
the linewidth of dendrogram-extracted clumps for clipping assuming the
underlying source is gaussian following \citet{Rosolowsky2006a} Appendix B.}
$\sigma_v$, the
...
\todo{TODO: replace with despotic version.}

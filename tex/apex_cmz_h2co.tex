...
{To measure the temperature of the dense gas in the Central Molecular Zone
(CMZ) and determine what drives it.}
{We have mapped the inner 300 pc of the CMZ in the temperature-sensitive
$J=3-2$ para-formaldehyde (\para) transitions.}
{We produce temperature maps and cubes and publish all data in FITS form; they
can be recovered from digital object identifier \texttt{doi:10.7910/DVN/27601}.
...
heating, though cosmic ray heating may also be significant.  We infer an upper
limit on the cosmic ray ionization rate $\zeta_{CR}<10^{-14}$ \pers. }
{
The bulk molecular gas temperatures of the region around our Galactic center
($T\sim 65$K) are similar to values found in the central regions of other
galaxies, in particular starburst systems.  Turbulent heating can readily
...
regions, the population of the higher energy states \emph{may} be affected by
`formation heating', a mechanism recently discovered to affect much of the CMZ
gas in which the excess energy from molecule formation leaves a significant
fraction of the molecules in a rotationally excited state,
\citep[][]{Mills2013a,Lis2014a} so confirmation of high temperatures seen in
\ammonia can be used to assess the significance of that mechanism.
\citet{Ao2013a} used the para-formaldehyde (\para) 220 GHz thermometer
...
extinction.  Other processes, e.g. direct (proto)stellar heating or supernova bubble
interaction may be relevant locally, but cannot heat the gas globally.
\citet{Ao2013a} ruled out diffuse X-rays as a significant heating source, with
an energy density $\sim3$ orders of magnitude too low to explain observed
temperatures.
...
It is clear from these plots that no single cosmic ray ionization rate is
capable of reproducing our observations, and the higher CRIRs ($\zeta_{CR} >
10^{-14}$ \pers) are inconsistent with much of the data.
}
...
There are few points with measured temperatures higher than predicted, but the
regions with predicted temperatures higher than observed require additional
...
fitting analysis with systematic blob extraction in order to avoid using the
...
We would also expect cooler interiors the clouds are centrally condensed, since
...
the CMZ.  For The Brick, our data seem to support the P Cygni interpretation of
...
interior of the cloud, while the gas temperature is set by turbulence.
...
over the first $\sim0.5$ Myr supports the \citet{Kruijssen2015a} hypothesis
...
\citep[][Figure 1]{Mangum2013a}.  The \ammonia lines also have lower critical
densities by a few orders of magnitude,
...
while $n_{eff}(\para~\threeohthree)\sim1\ee{5}$ \citep[][]{Shirley2015a}.  The
temperatures derived from the \ammonia line ratio are lower for a variety of
reasons.  The lower critical density means that \ammonia is measuring more
diffuse gas (which is apparently colder) that is not detected at all in
\formaldehyde.  Sensitivity also plays a role, since \formaldehyde only
provides measurable temperatures above $\sim50$ K in most locations.
...
We interpret the measured ratios differently.  Above 300K, the collision rates
provided by LAMDA \citep{Green1991a,Schoier2005a,Wiesenfeld2013a} must be
extrapolated, which means the models do not produce reliable results.
Very high $R_1$ values above about 0.7 cannot be reproduced by
high temperatures with the current suite of collision rates if the temperature
is bounded to $T<300$K, but instead require that the lines be optically thick
and thermalized and therefore that the gas be at very high densities.   Given
the low brightness temperatures observed ($\sim2$ K even in the SMA $\sim4$\arcsec
beams), if the emission is optically thick, it must be coming from an extremely
small area (it must have a small filling factor).  A small filling factor for
the cloud-scale APEX observations is inconsistent with the observed large
extent of the emission and the low interferometer to single-dish ratio.  We
discuss this topic further in Section \ref{sec:thickorwarm}.  We conclude,
therefore, that a lower limit on the gas temperature in The Brick,
$T\gtrsim120$ K, is consistent with our observations, and that the
\citet{Johnston2014a} measurement should also be regarded as a lower limit
closer to $T>300$ K or a suggestion that there is optically thick \para
\threeohthree emission at the brightness peaks.
